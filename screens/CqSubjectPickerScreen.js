...
export default function CqSubjectPickerScreen({ navigation }) {
    const { t } = useLanguage();
    ...
    const load = useCallback(async () => {
        ...
        const { data: chaptersData } = await supabase
            .from('chapters')
            .select('subject_id, cq_videos(count), subjects!inner(class_level_id)')
            .eq('subjects.class_level_id', classLevelId);

        const nonEmptySubjectIds = new Set(
            (chaptersData || [])
                .filter((c) => (c.cq_videos?.[0]?.count || 0) > 0)
                .map((c) => c.subject_id)
        );
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.header}>
                <Text style={styles.largeTitle}>{t('cq')}</Text>
            </View>
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <Pressable
                            ...
                            onPress={() => navigation.navigate('CqChapterList', { subject: item })}
                        >
                    ...
                    ListEmptyComponent={
                        <View style={styles.emptyState}>
                            ...
                            <Text style={styles.emptySubtitle}>{t('noCqVideosYet')}</Text>
                        </View>
                    }
                />
            )}
