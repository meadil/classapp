...
export default function CqChapterListScreen({ navigation, route }) {
    const { t } = useLanguage();
    ...
    const load = useCallback(async () => {
        ...
        const { data } = await supabase
            .from('chapters')
            .select('*, cq_videos(count)')
            .eq('subject_id', subject.id)
            .order('order_index', { ascending: true });

        setChapters((data || []).filter((c) => (c.cq_videos?.[0]?.count || 0) > 0));
        setLoading(false);
    }, [subject.id]);
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <Pressable
                            ...
                            onPress={() => navigation.navigate('CqVideoList', { chapter: item })}
                        >
                    ...
                    ListEmptyComponent={
                        <View style={styles.emptyState}>
                            ...
                            <Text style={styles.emptySubtitle}>{t('noCqVideosHere')}</Text>
                        </View>
                    }
                />
            )}
