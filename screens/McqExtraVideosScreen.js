...
export default function McqExtraVideosScreen({ navigation, route }) {
    const { t } = useLanguage();
    ...
    const load = useCallback(async () => {
        setLoading(true);
        const { data, error } = await supabase.from('mcq_extra_videos').select('*').eq('chapter_id', chapter.id).order('order_index', { ascending: true });
    ...
    const handleLongPress = (video) => {
        alert(video.title, undefined, [
            { text: t('edit'), onPress: () => navigation.navigate('CreateMcqExtraVideo', { video, chapterId: chapter.id }) },
            {
                text: t('delete'),
                style: 'destructive',
                onPress: async () => {
                    const { error } = await supabase.from('mcq_extra_videos').delete().eq('id', video.id);
    ...
    const persistOrder = async (reordered) => {
        ...
        const results = await Promise.all(
            reordered.map((item, index) => supabase.from('mcq_extra_videos').update({ order_index: index }).eq('id', item.id))
        );
    ...
    return (
        <SafeAreaView style={styles.container}>
            <View style={styles.header}>
                ...
                <Text style={styles.headerTitle} numberOfLines={1}>{t('extraVideos')}</Text>
                <Pressable onPress={() => navigation.navigate('CreateMcqExtraVideo', { chapterId: chapter.id })}>
                    ...
                    ListEmptyComponent={
                        !loading ? (
                            <View style={styles.emptyState}>
                                <Text style={styles.emptyTitle}>{t('noExtraVideosYet')}</Text>
                                <Text style={styles.emptySubtitle}>{t('noExtraVideosSub')}</Text>
                            </View>
                        ) : null
                    }
